...
class YOLODataset(Dataset):
  ...
  def __getitem__(self, index):
    ...
    #return image, 0.0,  box_coordinates
    label=0
    image = np.array(image) / 255.0
    #image = np.transpose(image, (2, 0, 1))
    #print(image.shape)
    #return torch.FloatTensor(image), torch.tensor(label, dtype=torch.long), torch.FloatTensor([x1, y1, x2, y2])
...
class SimplifiedSSD(nn.Module):
    ...
    def forward(self, x):
        #x = self.feature_extractor(x) # TAKE OFF ; not possible adaptation to models.vgg16
        x = self.avgpool(x)
        #print(x.shape)
        x = torch.flatten(x, 1)
